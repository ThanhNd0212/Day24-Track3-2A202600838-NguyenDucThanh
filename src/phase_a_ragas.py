...
def _nanmean(values: list[float]) -> float:
    """Mean bỏ qua NaN (RAGAS trả NaN cho job lỗi API sau khi hết retry)."""
    clean = [v for v in values if v == v]  # v == v là False chỉ khi v là NaN
...
def save_phase_a_report(results: list[RagasResult], clusters: dict,
    ...
    for dist in ["factual", "multi_hop", "adversarial"]:
        ...
        if subset:
            per_dist[dist] = {
                "count": len(subset),
                "faithfulness":      _nanmean([r.faithfulness for r in subset]),
                "answer_relevancy":  _nanmean([r.answer_relevancy for r in subset]),
                "context_precision": _nanmean([r.context_precision for r in subset]),
                "context_recall":    _nanmean([r.context_recall for r in subset]),
                "avg_score":         _nanmean([r.avg_score for r in subset]),
            }
