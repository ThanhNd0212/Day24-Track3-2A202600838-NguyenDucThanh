...
from config import GOOGLE_API_KEY, JUDGE_MODEL, HUMAN_LABELS_PATH, TEST_SET_PATH
...
if __name__ == "__main__":
    # --- Judge model_answer vs ground_truth for the 10 human-labeled questions ---
    ...
    print(f"Running LLM judge on {len(human_data)} human-labeled questions...")
    judge_results: list[JudgeResult] = []
    judge_labels: list[int] = []
    for item in human_data:
        gt = ground_truths[item["question_id"]]
        # A = model_answer (cần đánh giá), B = ground_truth (tham chiếu đúng)
        result = swap_and_average(item["question"], item["model_answer"], gt)
        judge_results.append(result)
        # model_answer được coi là "đúng" nếu judge cho A thắng hoặc tie với ground truth
        ...
        print(f"  Q{item['question_id']}: judge={judge_labels[-1]} human={item['human_label']} "
              f"(final_winner={result.final_winner}, consistent={result.position_consistent})")

    human_labels = [item["human_label"] for item in human_data]
    kappa = cohen_kappa(judge_labels, human_labels)
    print(f"\nCohen's κ (judge vs human): {kappa:.3f}")
